...
final class SavedMixesService {
    ...
    func setAnalyticsService(_ service: AnalyticsService) {
        ...
        analyticsService?.setSavedMixesCount(mixes.count)
    }
    ...
    func saveMix(name: String, sounds: [ActiveSound]) {
        ...
        // Log analytics event
        analyticsService?.logMixSave(
            mixName: name,
            soundCount: sounds.count,
            soundIds: sounds.map { $0.id }
        )
        analyticsService?.setSavedMixesCount(mixes.count)
    }
    ...
    func deleteMix(_ mix: SavedMix) {
        ...
        // Log analytics event
        analyticsService?.logEvent(.mixDelete, parameters: ["mix_name": mix.name])
        analyticsService?.setSavedMixesCount(mixes.count)
    }
